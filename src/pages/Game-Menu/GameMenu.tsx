...
import type { MapListItem } from "../../types/api/cms/maps";

export default function GameMenu() {
  const [levels, setLevels] = useState<MapListItem[]>([]);
  ...
  useEffect(() => {
    const loadLevels = async () => {
      try {
        ...
        // API returns paginated data in response.data.data
  ...
  const handleLevelSelect = (level: MapListItem) => {
    // Navigate to appropriate game view based on level title (infer type)
    // Check if title contains "platform" to determine game type
    const isPlatform = level.title.toLowerCase().includes("platform");
    const route = isPlatform ? "/platform" : "/game";
  ...
  const getTypeIcon = (title: string) => {
    ...
    return title.toLowerCase().includes("platform") ? "🎮" : "🎯";
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-900 to-purple-900 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {levels.map((level) => {
            const isPlatform = level.title.toLowerCase().includes("platform");
            const gameType = isPlatform ? "platformer" : "topdown";
            return (
              <div
                key={level.id}
              ...
              >
                ...
                <div className="bg-gradient-to-r from-blue-500 to-purple-600 p-6 text-center">
                  <div className="text-6xl mb-2">{getTypeIcon(gameType)}</div>
                  <div className="text-white text-sm uppercase tracking-wider">{gameType}</div>
                </div>
