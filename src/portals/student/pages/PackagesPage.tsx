// src/portals/student/pages/PackagesPage.tsx
import { useEffect, useState } from "react";
import { studentPackagesApi } from "@/services/api/student/packages.api";
import type { Package } from "@/types/api/student/packages";
...
export default function PackagesPage() {
  ...
  const loadPackages = async (page: number) => {
    try {
      setLoading(true);
      setError(null);
      const response = await studentPackagesApi.getAll(page, 20);
  ...
  const loadPackageDetail = async (id: string) => {
    try {
      const response = await studentPackagesApi.getById(id);
      if (response.data.isSuccess && response.data.data) {
